...
func main() {
	...
	m.Get("login", routes.AccessLimiter, routes.Login)
	m.Post("open", routes.AccessLimiter, binding.MultipartForm(routes.XcxConfigForm{}), routes.ValidErrorHandler, routes.OpenProject)
	m.Get("preview", routes.AccessLimiter, binding.Bind(routes.XcxBaseForm{}), routes.Preview)
	m.Get("upload", routes.AccessLimiter, binding.Bind(routes.XcxBaseForm{}), routes.Upload)
